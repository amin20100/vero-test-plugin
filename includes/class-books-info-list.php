...
class Books_Info_List extends WP_List_Table
{
	public function __construct() {
		parent::__construct( [
			'singular' => __( 'Info', 'dw-books' ),
			'plural'   => __( 'Info', 'dw-books' ),
			'ajax'     => false
        ] );
	...
	function column_ID( $item ) {
		...
		$actions = [
			'delete' => sprintf( '<a href="?post_type=book&page=%s&action=%s&info=%s&_wpnonce=%s">Delete</a>', esc_attr( $_REQUEST['page'] ), 'delete', absint( $item['ID'] ), $delete_nonce )
		];
    ...
    function column_book($item) {
        echo '<a target="_blank" href="' . get_edit_post_link($item['post_id']) . '">'. get_the_title($item['post_id']) .'</a>';
    }
